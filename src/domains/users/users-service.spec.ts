...
describe('UsersService', () => {
    ...
    describe('findOneByEmail', () => {
        ...
        it('should throw when user is not find', async () => {
            ...
            await expect(sut.findOneByEmail(email)).rejects.toThrow(NotFoundException);
            expect(usersRepository.findOneByEmail).toHaveBeenCalledWith(email);
        });
    });
    ...
    describe('findByHashedToken', () => {
        ...
        it('should find a user by token', async () => {
            ...
            jest.spyOn(usersRepository, 'findOneByHashedToken').mockResolvedValueOnce(user);
        ...
        it('should throw when user is not find', async () => {
            ...
            jest.spyOn(usersRepository, 'findOneByHashedToken').mockResolvedValueOnce(null);
            await expect(sut.findByHashedToken(token)).rejects.toThrow(UnauthorizedException);
            expect(usersRepository.findOneByHashedToken).toHaveBeenCalledWith(token);
        });
    });
});